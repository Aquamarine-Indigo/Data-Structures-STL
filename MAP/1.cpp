...
void tester(void) {
	...
	// std::cout << "Left_end = " << map.left_end << std::endl;
	// std::cout << "Right_end = " << map.right_end << std::endl;
	// map.printtree();
	while (const_iterator != map.cend()) {
		const Integer integer(const_iterator->first);
		// std::cout << "pointer = " << const_iterator.pointer << std::endl;
		const_iterator++;
		std::cout << map.at(integer);// << " " << "\tnext\t";
	}
	std::cout << std::endl;
