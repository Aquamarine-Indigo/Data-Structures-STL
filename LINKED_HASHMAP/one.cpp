...
void tester(void) {
	...
	std::cout << std::endl;
	// std::cerr << "test iterator\n";
	// std::cerr << "find 2016: " << map.count(Integer(2016)) << std::endl;
	...
	const_iterator = map.cbegin();
		// sjtu::linked_hashmap<Integer, std::string,Hash,Equal>::const_iterator enditer = map.cend();
		// std::cout << "end: (" << enditer.pt << ") ";
	while (const_iterator != map.cend()) {
		const Integer integer(const_iterator->first);
		const_iterator++;
		std::cout << map.at(integer) << " ";
		// std::cout << "(" << const_iterator->first.val << ", " << map[integer] << ") ";
	}
	std::cout << std::endl;
