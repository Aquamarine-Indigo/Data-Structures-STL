...
bool testException() {
	...
	std::cerr << "--- Construct complete\n";
    try{ myList.pop_back(); } catch (...) { std::cerr << "1\n"; ans++; }
    try{ myList.pop_front(); } catch (...) { std::cerr << "2\n"; ans++; }
	std::cerr << "--- Pop complete\n";
    try{ myList.front(); } catch (...) { std::cerr << "3\n"; ans++; }
    try{ myList.back(); } catch (...) { std::cerr << "4\n"; ans++; }
	std::cerr << "--- Access complete\n";
    sjtu::list<int>::iterator it = myList.end(), oit = otherList.end();
    try{ *it; } catch (...) { std::cerr << "5\n"; ans++; }
    try{ it--; } catch (...) { std::cerr << "6\n"; ans++; }
    try{ it++; } catch (...) { std::cerr << "7\n"; ans++; }
    std::cerr << "--- iterator complete\n";
    try{ myList.erase(it); } catch (...) { std::cerr << "8\n"; ans++; }
    try{ myList.insert(oit, 0); } catch (...) { std::cerr << "9\n"; ans++; }
    std::cerr << "--- insert/erase complete\n";
